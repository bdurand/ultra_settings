...
describe SuperConfig::Field do
  describe "value hierarchy" do
    let(:field) { SuperConfig::Field.new(name: "foo") }
    ...
    it "does not use the environment if it is disabled", env: {FOO: "env"}, settings: {foo: "setting"} do
      field = SuperConfig::Field.new(name: "foo")
  ...
  describe "env_var_prefix" do
    it "adds a prefix to the environment variable name", env: {PRE_FOO: "bar"} do
      field = SuperConfig::Field.new(name: "foo", env_var_prefix: "PRE_")
  ...
  describe "setting_prefix" do
    it "adds a prefix to the setting name", settings: {pre_foo: "bar"} do
      field = SuperConfig::Field.new(name: "foo", setting_prefix: "pre_")
      expect(field.value).to eq("bar")
    end
  end
  ...
  describe "env_var" do
    it "gets a value from the environment variable", env: {VALUE: "bar"} do
      field = SuperConfig::Field.new(name: "foo", env_var: :VALUE, env_var_prefix: "X_")
      expect(field.value).to eq("bar")
    end
  end
  ...
  describe "setting_name" do
    it "gets a value from the setting name", settings: {value: "bar"} do
      field = SuperConfig::Field.new(name: "foo", setting_name: :value, setting_prefix: "x_")
      expect(field.value).to eq("bar")
    end
  end
  ...
  describe "yaml_key" do
    it "gets a value from YAML key" do
      field = SuperConfig::Field.new(name: "foo", yaml_key: :value)
  ...
  describe "default" do
    it "uses the default value if no other value is present" do
      field = SuperConfig::Field.new(name: "foo", default: "bar")
      expect(field.value).to eq("bar")
    end

    it "does not use the default value if the value is false", settings: {foo: false} do
      field = SuperConfig::Field.new(name: "foo", type: :boolean, default: true)
    ...
    it "coerces the default value to the specified type" do
      field = SuperConfig::Field.new(name: "foo", type: :integer, default: "1")
  ...
  describe "type" do
    describe "string" do
      it "coerces the value to a string" do
        field = SuperConfig::Field.new(name: "foo", type: :string)
    ...
    describe "boolean" do
      it "coerces the value to a boolean", env: {FOO: "true", BAR: "0"} do
        field = SuperConfig::Field.new(name: "foo", type: :boolean)
        expect(field.value).to be true

        field = SuperConfig::Field.new(name: "bar", type: :boolean)
        expect(field.value).to be false
      end
    ...
    describe "integer" do
      it "coerces the value to an integer", env: {FOO: "1"} do
        field = SuperConfig::Field.new(name: "foo", type: :integer)
        expect(field.value).to eq(1)
      end
    ...
    describe "float" do
      it "coerces the value to a float", env: {FOO: "1.1"} do
        field = SuperConfig::Field.new(name: "foo", type: :float)
    ...
    describe "datetime" do
      it "coerces the value to a datetime", env: {FOO: "2015-01-01 12:01:50Z"} do
        field = SuperConfig::Field.new(name: "foo", type: :datetime)
    ...
    describe "array" do
      it "coerces the value to an array of strings" do
        field = SuperConfig::Field.new(name: "foo", type: :array)
